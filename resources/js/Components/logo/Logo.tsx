...
export default function Logo({ isCompact = true }: { isCompact: boolean }) {
    ...
    const isOnTop = (animationState === "compact_end" || animationState === "wide_start")
                ...
                >
                    <Link href="/">
                        <img src={`/assets/images/logo-images/souper.svg`} alt="Souper"
                            className={`absolute ${isOnTop ? "w-[45%] top-0 left-0" : "w-[80%] top-0 left-[20%"} transition-all will-change-transform duration-700 ease-out `} />
                        <img src={`/assets/images/logo-images/lopers.svg`} alt="Lopers"
                            className={`absolute ${isOnTop ? "w-[43.76%] bottom-0 left-[45%]" : "w-[80%] bottom-0 left-0"} transition-all will-change-transform duration-700 ease-out `} />
                        <img src={`/assets/images/logo-images/ptx.svg`} alt="Gul"
                            className={`absolute ${isOnTop ? "w-[11.24%] bottom-1/2 translate-y-1/2 right-0" : "w-[8%] bottom-[5%] right-[5%] rotate-[80deg]"} transition-all will-change-transform duration-700 ease-out `} />
                    </Link>
                </motion.div>
            </AnimatePresence>,
            target,
